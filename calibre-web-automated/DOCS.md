...
3. **Set `network_share_mode: true`** — required for SMB/NFS to disable SQLite WAL (which corrupts on network shares) and switch the ingest watcher from inotify to polling.
4. **Migrating from an existing CWA install**:
   ...
   - To preserve user accounts and CWA's app database, stop the add-on, then via Samba copy your old CWA `/config` contents into `/addon_configs/<addon-slug>_calibre-web-automated/` (where the slug is shown in the add-on's "Info" tab).
   - Delete the sentinel: `rm /addon_configs/.../.cwa-initialized` so the add-on doesn't think it's already initialized.
   - Restart the add-on.
...
The add-on stores CWA's app database, user accounts, and settings under `/addon_configs/<slug>_calibre-web-automated/` (mapped from `/data` inside the container). This survives add-on upgrades and uninstalls. Library files in `library_path` (e.g. under `/share`) are independent of the add-on lifecycle.
